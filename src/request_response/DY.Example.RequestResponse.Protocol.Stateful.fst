...
let client_send_request comm_keys_ids client server =
  ...
  let payload = compute_message client nonce in
...
let server_receive_request_send_response comm_keys_ids server msg_id =
  let*? (req_bytes, req_meta_data) = receive_request comm_keys_ids server msg_id in
  let*? req:request = return (decode_message req_bytes) in
  ...
  let*? msg_id = send_response comm_keys_ids server req_meta_data req.nonce in
  return (Some msg_id)
...
let client_receive_response comm_keys_ids client sid msg_id =
  ...
  let*? (res_bytes, _) = receive_response comm_keys_ids client cmeta_data msg_id in
  guard_tr (res_bytes = nonce);*?
