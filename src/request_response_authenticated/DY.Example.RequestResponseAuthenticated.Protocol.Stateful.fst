module DY.Example.RequestResponseAuthenticated.Protocol.Stateful
...
let client_send_request comm_keys_ids client server =
  ...
  let*? (msg_id, cmeta_data) = send_request_authenticated comm_keys_ids client server payload in
...
let server_receive_request_send_response comm_keys_ids server msg_id =
  let*? (msg, req_meta_data) = receive_request_authenticated comm_keys_ids server msg_id in
  guard_tr (Request? msg);*?
  let Request req = msg in
  let* sid = new_session_id server in
  set_state server sid (ServerReceiveRequest { client=Some?.v req_meta_data.client; nonce=req.nonce } <: protocol_state);*
