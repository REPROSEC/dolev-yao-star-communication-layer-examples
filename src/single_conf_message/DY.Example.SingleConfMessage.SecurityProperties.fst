...
// This is the main security property of a confidential message. It states from
// the sender perspective that the secret is not leaked to the attacker as long
// as the sender and receiver are honest.
//
// **Note:** We know that if the receiver is honest only the receiver can
// decrypt the secret. This allows us to argue from the sender's perspective
// that if the attacker knows the secret either the sender or the receiver is
// corrupt.
...
// On the receiver side we don't have any guarantees on the secrecy of the
// secret. We can only prove that if the attacker knows the secret, it is
// publishable.
...
// We can also not authenticate the sender because the attacker could have
// encrypted the message The only thing we can prove is that if the secret is
// not publisable, there exists a sender that triggered the send event.
val sender_authentication:
  ...
  (ensures
    ...
    is_publishable tr (serialize single_message msg)
    // We cannot prove `is_corrupt tr (principal_label sender)` because the
    // sender is not authenticated. If the attacker creates this message, they
    // can choose an arbitrary sender for the sender field.
  )
