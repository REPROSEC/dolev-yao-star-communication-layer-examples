module DY.Example.SingleConfMessage.Protocol.Stateful
...
type login_state =
  ...
  | ServerState: secret:bytes -> payload:bytes -> login_state
...
type login_event =
  ...
  | ServerReceivedMsg: client:principal -> server:principal -> secret:bytes -> payload:bytes -> login_event
...
let receive_message comm_keys_ids server msg_id =
  let*? msg:communication_message_base = receive_confidential comm_keys_ids server msg_id in
  let*? single_msg:single_message = return (decode_message msg.payload) in
  trigger_event server (ServerReceivedMsg msg.sender server single_msg.secret msg.payload);*
  let* state_id = new_session_id server in
  set_state server state_id (ServerState single_msg.secret msg.payload <: login_state);*
